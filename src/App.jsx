...
function App() {
  ...
  return (
    <AppLayout currentView={currentView} onNavigate={handleNavigate}>
      ...
      {currentView === 'inventory' && <DeviceInventory onDeviceClick={(id) => handleNavigate('device-details', id)} />}
      {currentView === 'monitoring' && <LiveMonitoring onDeviceClick={(id) => handleNavigate('device-details', id)} />}
      {currentView === 'device-details' && <DeviceDetails deviceId={activeDeviceId} onBack={() => handleNavigate('inventory')} />}
