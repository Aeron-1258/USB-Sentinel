...
const initialAllowlist = [
  { id: 'AL-1', type: 'Security', vendor: 'Yubico', serial: 'YB-8921-009', addedBy: 'admin@company.com', addedTime: 'Oct 12, 2025', notes: 'Standard issue security key for Engineering department.', risk: 'Low' },
  { id: 'AL-2', type: 'HID', vendor: 'Logitech', serial: 'LOG-KBD-991', addedBy: 'admin@company.com', addedTime: 'Sep 05, 2025', notes: 'Approved wireless receiver for standard keyboards.', risk: 'Low' },
  { id: 'AL-3', type: 'Storage', vendor: 'Kingston', serial: 'KN-DT50-128', addedBy: 'secops@company.com', addedTime: 'Aug 22, 2025', notes: 'Encrypted drive for authorized offline backups. Restricted to SOC team.', risk: 'Medium' },
];

const initialBlocklist = [
  { id: 'BL-1', type: 'Storage', vendor: 'SanDisk', serial: 'SD-CRZ-001', addedBy: 'SYSTEM', addedTime: 'Today, 14:22', notes: 'Blocked via automated Mass Storage Policy V3. Unauthorized data exfiltration risk.', risk: 'High' },
  { id: 'BL-2', type: 'Unknown', vendor: 'Generic', serial: 'UNK-8829-FF', addedBy: 'SYSTEM', addedTime: 'Yesterday, 09:11', notes: 'BadUSB signature detected. Quarantined indefinitely.', risk: 'High' },
];
...
export default function PolicyManagement() {
  ...
  const filteredList = currentList.filter(device => 
  ...
  const handleApprove = (id) => {
    ...
    if (activeTab === 'blocklist') {
      ...
      setAllowlist(prev => [{...device, risk: 'Low', notes: 'Manually approved by admin.', addedBy: 'admin@company.com'}, ...prev]);
    } else {
  ...
  return (
    <div className="flex-col gap-3" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="flex justify-between items-center" style={{ marginBottom: '8px' }}>
        <div>
          <h1 className="heading-1" style={{ margin: 0 }}>Device Policy Management</h1>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid var(--color-border-light)' }}>
        <button 
          onClick={() => { setActiveTab('allowlist'); setSelectedIds([]); }}
          style={{ 
            padding: '12px 24px', 
            background: 'none', 
            border: 'none', 
            borderBottom: activeTab === 'allowlist' ? '3px solid var(--color-blue-500)' : '3px solid transparent',
            color: activeTab === 'allowlist' ? 'var(--color-blue-600)' : 'var(--color-text-secondary)',
        ...
        <button 
          onClick={() => { setActiveTab('blocklist'); setSelectedIds([]); }}
          style={{ 
            padding: '12px 24px', 
            background: 'none', 
            border: 'none', 
            borderBottom: activeTab === 'blocklist' ? '3px solid var(--color-blue-500)' : '3px solid transparent',
            color: activeTab === 'blocklist' ? 'var(--color-blue-600)' : 'var(--color-text-secondary)',
      ...
      <div className="card" style={{ padding: '12px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
          ...
          {selectedIds.length > 0 && (
            <div style={{ display: 'flex', gap: '8px', borderLeft: '1px solid var(--color-border-light)', paddingLeft: '24px' }}>
              <button className="btn btn-secondary" style={{ height: '32px', color: 'var(--color-red-600)', borderColor: 'var(--color-red-200)' }} onClick={handleBulkDelete}>
      ...
      {/* List Area */}
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px', paddingRight: '8px', paddingTop: '8px' }}>
        {filteredList.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '64px', color: 'var(--color-text-tertiary)' }}>
            <span className="material-symbols-rounded" style={{ fontSize: '48px', marginBottom: '16px' }}>policy</span>
