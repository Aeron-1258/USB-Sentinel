...
export default function LiveMonitoring({ onDeviceClick }) {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'Authorized': case 'OK': return <span className="badge badge-success">Authorized</span>;
      case 'Blocked': return <span className="badge badge-danger">Blocked</span>;
      case 'Quarantined': return <span className="badge badge-purple">Quarantined</span>;
      case 'Read Only': return <span className="badge badge-warning">Read-Only</span>;
      default: return <span className="badge badge-success">Connected</span>;
    }
  };
    ...
    <div className="flex-col gap-3" style={{ height: '100%', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="flex justify-between items-center" style={{ marginBottom: '4px', flexShrink: 0 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <h1 className="heading-1" style={{ margin: 0 }}>Live USB Monitoring</h1>
            {mode === 'LIVE' ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', backgroundColor: 'var(--color-green-50)', color: 'var(--color-green-600)', padding: '4px 12px', borderRadius: '16px', fontSize: '12px', fontWeight: 600 }}>
                <span className="material-symbols-rounded pulse" style={{ fontSize: '14px' }}>fiber_manual_record</span>
                LIVE ENTERPRISE MODE
              </div>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', backgroundColor: 'var(--color-yellow-50)', color: 'var(--color-yellow-700)', padding: '4px 12px', borderRadius: '16px', fontSize: '12px', fontWeight: 600 }}>
                <span className="material-symbols-rounded" style={{ fontSize: '14px' }}>play_circle</span>
                DEMO MODE (PREVIEW)
              </div>
            )}
          </div>
          <p className="text-subtitle" style={{ margin: '2px 0 0 0' }}>Real-time PnP hardware telemetry, forensic connection analysis, and instant security enforcement.</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: 'var(--space-2)', flexShrink: 0 }}>
        ...
        <div className="card" style={{ padding: '12px 14px' }}>
          ...
          <div style={{ fontSize: '20px', fontWeight: 700, marginTop: '4px', color: 'var(--color-green-600)' }}>{authorizedCount}</div>
        </div>
        <div className="card" style={{ padding: '12px 14px' }}>
        ...
        <div className="card" style={{ padding: '12px 14px' }}>
          ...
          <div style={{ fontSize: '20px', fontWeight: 700, marginTop: '4px', color: 'var(--color-yellow-600)' }}>{unknownCount}</div>
        </div>
        <div className="card" style={{ padding: '12px 14px' }}>
        ...
        <div className="card" style={{ padding: '12px 14px' }}>
          <div style={{ fontSize: '11px', color: 'var(--color-text-tertiary)', fontWeight: 600, textTransform: 'uppercase' }}>Live Events</div>
          <div style={{ fontSize: '20px', fontWeight: 700, marginTop: '4px' }}>{eventCount}</div>
        </div>
        <div className="card" style={{ padding: '12px 14px' }}>
          <div style={{ fontSize: '11px', color: 'var(--color-green-600)', fontWeight: 600, textTransform: 'uppercase' }}>Security Score</div>
          <div style={{ fontSize: '20px', fontWeight: 700, marginTop: '4px', color: 'var(--color-green-600)' }}>{overallSecurityScore}/100</div>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div style={{ width: '220px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '8px', backgroundColor: 'var(--color-blue-50)', color: 'var(--color-blue-600)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div style={{ width: '140px', fontFamily: 'monospace', color: 'var(--color-text-secondary)', fontSize: '12px' }}>
                  <div>VID: {dev.vid}</div>
                  <div>PID: {dev.pid}</div>
                </div>

                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px', color: 'var(--color-text-secondary)', fontSize: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <span className="material-symbols-rounded" style={{ fontSize: '16px' }}>hard_drive</span>
                      {mountPoint} ({fileSystem})
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <span className="material-symbols-rounded" style={{ fontSize: '16px' }}>sd_storage</span>
                      {capacity}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <span className="material-symbols-rounded" style={{ fontSize: '16px' }}>speed</span>
                      SuperSpeed (USB 3.0)
                    </div>
                  </div>
                ...
                <div style={{ width: '140px' }}>
                  {getStatusBadge(dev.status || 'Authorized')}
                </div>

                <div style={{ width: '140px', display: 'flex', gap: '4px', justifyContent: 'flex-end' }} onClick={(e) => e.stopPropagation()}>
                  <button className="btn-icon" title="👁 View Details" onClick={() => setSelectedDevice(dev)}>
                    <span className="material-symbols-rounded" style={{ fontSize: '18px' }}>visibility</span>
                  </button>
                  <button className="btn-icon" style={{ color: 'var(--color-green-600)' }} title="✅ Allow Device">
                    <span className="material-symbols-rounded" style={{ fontSize: '18px' }}>check_circle</span>
                  </button>
                  <button className="btn-icon" style={{ color: 'var(--color-red-600)' }} title="🚫 Block Device">
