...
export default function DeviceInventory({ onDeviceClick }) {
  ...
  const handleExportCSV = () => {
    const headers = [
      "Device Name,Manufacturer,Class,VID,PID,Serial,HardwareID,Status,TrustScore,RiskScore,MountPoint\n",
    ];
  ...
  const filteredDevices = devices.filter((dev) => {
    const matchesSearch = Object.values(dev).some(
      (val) =>
        val !== null &&
        val !== undefined &&
    ...
    const matchesRisk =
      riskFilter === "All" || (dev.riskScore > 50 ? "High" : "Low") === riskFilter;
      ...
      >
        <div>
          ...
          <p className="text-subtitle" style={{ margin: "2px 0 0 0" }}>
            Real-time inventory of all USB and Plug-and-Play devices connected to this endpoint.
          </p>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <div style={{ fontSize: "12px", color: "var(--color-text-tertiary)" }}>
            Last Scan:{" "}
            <strong style={{ color: "var(--color-text-secondary)" }}>{lastScanTime}</strong>
          </div>
          <button className="btn btn-secondary" onClick={handleManualRefresh} disabled={isScanning}>
            <span
              className={`material-symbols-rounded ${isScanning ? "pulse" : ""}`}
              style={{ fontSize: "18px" }}
            >
              refresh
            </span>{" "}
      ...
      >
                ...
                <tr>
                  ...
                  >
                    ...
                    <div style={{ fontSize: "13px", marginTop: "4px" }}>
                      Plug in a physical USB device to see real-time endpoint telemetry.
                    </div>
                    <button
              ...
              {currentRows.map((dev) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: "12px 16px" }}>{getRiskBadge(riskScore)}</td>
                    <td
                    ...
                    >
                      <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end" }}>
                        ...
                        <button
                          ...
                          onClick={() =>
                            navigator.clipboard.writeText(dev.hardwareId || `${dev.vid}:${dev.pid}`)
                          }
                        >
        ...
        >
          <div style={{ fontSize: "12px", color: "var(--color-text-secondary)" }}>
            Showing {sortedDevices.length > 0 ? startIndex + 1 : 0} to{" "}
            {Math.min(startIndex + rowsPerPage, sortedDevices.length)} of {sortedDevices.length}{" "}
            assets
          </div>
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <span
                className="material-symbols-rounded"
                style={{ color: "var(--color-blue-600)", fontSize: "24px" }}
              >
                usb
              </span>
              <div>
                <h3 style={{ margin: 0, fontSize: "15px", fontWeight: 600 }}>
                  Asset Forensics Drawer
                </h3>
                <span
                  style={{
                    fontSize: "11px",
          ...
          >
            ...
            <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
              ...
              <div>
                <div
                  style={{ fontSize: "16px", fontWeight: 700, color: "var(--color-text-primary)" }}
                >
                  {selectedDevice.name}
                </div>
                <div style={{ fontSize: "12px", color: "var(--color-text-secondary)" }}>
                  {selectedDevice.manufacturer || "Generic Manufacturer"}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              <span className={`badge ${getStatusBadgeClass(selectedDevice.status || "OK")}`}>
                {selectedDevice.status || "OK"}
              </span>
              <span className="badge badge-neutral">Class: {selectedDevice.class || "USB"}</span>
              {selectedDevice.vid === "Unknown" ? (
                <span className="badge badge-danger">Unrecognized</span>
              ) : (
                <span className="badge badge-success">Signed Driver</span>
              )}
            </div>
              ...
              <div className="card" style={{ padding: "12px", fontSize: "12px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", padding: "4px 0" }}>
                  ...
                  <span
                    ...
                    title={selectedDevice.hardwareId}
                  >
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button
                  className="btn btn-secondary"
                  style={{ flex: 1, color: "var(--color-green-600)" }}
                >
                ...
                <button
                  className="btn btn-secondary"
                  style={{ flex: 1, color: "var(--color-red-600)" }}
                >
                  <span className="material-symbols-rounded" style={{ fontSize: "16px" }}>
                    block
                  </span>{" "}
