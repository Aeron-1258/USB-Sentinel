...
export function DataMovementChart() {
  return (
    <div className="card" style={{ flex: 1, height: '300px', display: 'flex', flexDirection: 'column' }}>
      <h3 style={{ margin: '0 0 16px 0', fontSize: '14px', color: 'var(--color-text-secondary)' }}>Data Movement (GB)</h3>
      <div style={{ flex: 1, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)' }} itemStyle={{ color: 'var(--color-text-primary)' }} />
            <Area type="monotone" dataKey="dataGB" stroke="var(--color-blue-500)" strokeWidth={2} fillOpacity={1} fill="url(#colorData)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
...
export function TrustScoreTrendChart() {
  return (
    <div className="card" style={{ flex: 1, height: '300px', display: 'flex', flexDirection: 'column' }}>
      <h3 style={{ margin: '0 0 16px 0', fontSize: '14px', color: 'var(--color-text-secondary)' }}>Average Trust Score Trend</h3>
      <div style={{ flex: 1, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)' }} itemStyle={{ color: 'var(--color-green-500)', fontWeight: 600 }} />
            <Line type="monotone" dataKey="trustScore" stroke="var(--color-green-500)" strokeWidth={3} dot={{ r: 4, fill: 'var(--color-green-500)' }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
...
export function DeviceStatusPieChart() {
  return (
    <div className="card" style={{ flex: 1, height: '300px', display: 'flex', flexDirection: 'column' }}>
      <h3 style={{ margin: '0 0 16px 0', fontSize: '14px', color: 'var(--color-text-secondary)' }}>Device Status Ratio</h3>
      <div style={{ flex: 1, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)' }} />
...
export function FileTypesPieChart() {
  return (
    <div className="card" style={{ flex: 1, height: '300px', display: 'flex', flexDirection: 'column' }}>
      <h3 style={{ margin: '0 0 16px 0', fontSize: '14px', color: 'var(--color-text-secondary)' }}>Top File Types Transferred</h3>
      <div style={{ flex: 1, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={fileTypeData} innerRadius={40} outerRadius={90} paddingAngle={2} dataKey="value" stroke="none">
              {fileTypeData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
            </Pie>
            <Tooltip contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)' }} />
            <Legend verticalAlign="bottom" height={36} iconType="circle" wrapperStyle={{ fontSize: '12px' }} />
          </PieChart>
...
export function TopUsersBarChart() {
  return (
    <div className="card" style={{ flex: 1, height: '300px', display: 'flex', flexDirection: 'column' }}>
      <h3 style={{ margin: '0 0 16px 0', fontSize: '14px', color: 'var(--color-text-secondary)' }}>Most Active Users</h3>
      <div style={{ flex: 1, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={topUsersData} layout="vertical" margin={{ top: 0, right: 10, left: 10, bottom: 0 }}>
            ...
            <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: 'var(--color-text-secondary)' }} />
            <Tooltip cursor={{ fill: 'var(--color-surface-hover)' }} contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }} />
...
export function TopVendorsBarChart() {
  return (
    <div className="card" style={{ flex: 1, height: '300px', display: 'flex', flexDirection: 'column' }}>
      <h3 style={{ margin: '0 0 16px 0', fontSize: '14px', color: 'var(--color-text-secondary)' }}>Top Hardware Vendors</h3>
      <div style={{ flex: 1, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={topVendorsData} layout="vertical" margin={{ top: 0, right: 10, left: 10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="var(--color-border-light)" />
            <XAxis type="number" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: 'var(--color-text-tertiary)' }} />
            <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: 'var(--color-text-secondary)' }} />
            <Tooltip cursor={{ fill: 'var(--color-surface-hover)' }} contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }} />
...
export function ActivityHeatMap() {
  ...
  const getColor = (intensity) => {
    switch (intensity) {
      case 0: return 'var(--color-bg-base)';
      case 1: return 'var(--color-blue-100)';
      case 2: return 'var(--color-blue-300)';
      case 3: return 'var(--color-blue-500)';
      case 4: return 'var(--color-blue-700)';
  ...
  return (
    <div className="card" style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column' }}>
      <h3 style={{ margin: '0 0 16px 0', fontSize: '14px', color: 'var(--color-text-secondary)' }}>Activity Heat Map (Connections per Hour)</h3>
      <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '8px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', paddingTop: '20px' }}>
          {days.map(d => <div key={d} style={{ height: '16px', fontSize: '11px', color: 'var(--color-text-tertiary)', display: 'flex', alignItems: 'center' }}>{d}</div>)}
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
            {hours.map(h => <div key={h} style={{ width: '16px', fontSize: '10px', color: 'var(--color-text-tertiary)', textAlign: 'center' }}>{h%2===0 ? h : ''}</div>)}
          </div>
