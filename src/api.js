...
const DEMO_AUDIT_LOGS = [
  { id: 'AUD-9001', timestamp: '2026-08-01 21:10:12', endpoint: 'DEMO-ENDPOINT-01', user: 'sec_analyst', process: 'explorer.exe (PID 4812)', action: 'File Copied to USB', device: 'SanDisk Cruzer', hash: 'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855', severity: 'Low' },
  { id: 'AUD-9002', timestamp: '2026-08-01 21:05:40', endpoint: 'DEMO-ENDPOINT-01', user: 'sec_analyst', process: 'services.exe (PID 812)', action: 'Driver Load Blocked', device: 'Kingston DataTraveler', hash: 'N/A', severity: 'High' }
];

const DEMO_FILE_EVENTS = [
  { id: 'FEV-101', timestamp: '2026-08-01 21:10:12', user: 'sec_analyst', filename: 'Q3_Financial_Audit.xlsx', action: 'File Written', source: 'C:\\Users\\Desktop\\Q3_Financial_Audit.xlsx', usbDevice: 'SanDisk Cruzer (E:)', hash: '8f434346648f6b96df89dda901c5176b10a6d83961dd3c1ac88b59b2dc327aa4' },
  { id: 'FEV-102', timestamp: '2026-08-01 20:45:00', user: 'sec_analyst', filename: 'Customer_DB_Export.csv', action: 'File Written', source: 'C:\\Exports\\Customer_DB_Export.csv', usbDevice: 'Kingston (F:)', hash: '4a0a19218e082a343a1b17e5333409af9d98f0f5f67a216e9195ba1a646c8209' }
];

const DEMO_ALERTS = [
  { id: 'ALT-501', timestamp: '2026-08-01 21:05:40', title: 'Unrecognized USB Storage Device Attached', severity: 'Critical', device: 'Kingston DataTraveler', vid: '0x0930', pid: '0x6545', user: 'sec_analyst', status: 'Active', mitreTechnique: 'T1200 Hardware Additions' }
];
...
export async function checkMode() {
  try {
    const res = await fetch(`${BASE_URL}/endpoint`, { method: 'GET', signal: AbortSignal.timeout(1500) });
    if (res.ok) {
      activeMode = 'LIVE';
      return 'LIVE';
    }
...
export async function triggerLiveScan() {
  try {
    const res = await fetch(`${BASE_URL}/scan`, { method: 'POST', signal: AbortSignal.timeout(1500) });
    if (res.ok) return await res.json();
  } catch (e) {}
